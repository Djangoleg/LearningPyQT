...
from common.variables import LOGGER_NAME_CLIENT, DEFAULT_IP_ADDRESS, DEFAULT_PORT, DEFAULT_CLIENT_NAME, \
    DEFAULT_CLIENT_PASS
...
class Client:
    ...
    def _get_settings(self):
        ...
        parser.add_argument(
            '-p',
            '--password',
            default=DEFAULT_CLIENT_PASS,
            nargs='?')
        namespace = parser.parse_args(sys.argv[1:])
        server_address = namespace.addr
        server_port = namespace.port
        client_name = namespace.name
        client_pwd = namespace.password
        ...
        return server_address, server_port, client_name, client_pwd
    ...
    def start(self):
        ...
        try:
            ...
            message = QMessageBox()
            message.critical(start_dialog, 'Ошибка сервера', str(error))
            exit(1)
