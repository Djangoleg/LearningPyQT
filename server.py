...
class Server(Thread, metaclass=ServerMaker):
    ...
    def __init__(self, config:configparser.ConfigParser, database:ServerStorage):
        ...
        if config['SETTINGS']['Default_port']:
            self.listen_port = int(config['SETTINGS']['Default_port'])
        else:
            self.listen_port = DEFAULT_PORT

        if config['SETTINGS']['Listen_Address']:
            self.listen_address = config['SETTINGS']['Listen_Address']
        else:
            self.listen_address = ''
    ...
    def process_client_message(self, message, client):
        ...
        # Если это запрос известных пользователей
                ...
                and self.names[message[ACCOUNT_NAME]] == client:
            response = RESPONSE_202
            response[LIST_INFO] = [user[0] for user in self.server_db.users_list()]
            send_message(client, response)
...
class ServerGui:
    ...
    def __init__(self, config:configparser.ConfigParser, database:ServerStorage):
        ...
        self.main_window.active_clients_table.setModel(gui_create_model(self.server_db))
